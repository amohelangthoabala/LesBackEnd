...
def create_message(request: schemas.CreateMessage, db: Session = Depends(get_db), current_user: schemas.User = Depends(auth.get_current_user)):
...
@router.get('/email')
def get_messages_owned_by_user(email, current_user: schemas.User = Depends(auth.get_current_user)):
